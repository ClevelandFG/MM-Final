...
## 2026-06-14  新增第 (4) 问参数网格批量重优化脚本

- **版本号**：未发布，仍处于第一个可行版本前。
- **问题**：第 (4) 问需要独立于 GUI 的命令行批量计算能力，用于对 `T`、`t`、`v` 全组合参数网格逐点重新优化，并为后续绘制热力图提供结构化数据。
- **解决方案**：
  - 新增 `scripts/run_parameter_batch.py`，默认固定 `k=3`，对 `T={1.0,1.5,2.0,2.5,3.0}`、`t={0.5,1.0,1.5}`、`v={25,30,35,40,45}` 的 `75` 组参数逐点运行固定组数最短完成时间优化。
  - 脚本显示 `已计算：i/75` 进度，并输出 `batch_results.json`、`batch_results.csv`、简短 `summary.md` 和每个参数点的 `RoutePlan` JSON。
  - 第 (4) 问目标口径明确为：固定组数下最小化最大单路线完成时间；当前结果为启发式重优化结果，若需强最优证明需另行补充证明。
- **影响文件**：`scripts/run_parameter_batch.py`、`docs/changes.md`。

---

## 2026-06-14  优化 GUI 路线显隐布局

- **版本号**：未发布，仍处于第一个可行版本前。
- **问题**：路线显隐复选框纵向排列，路线组较多时会挤占右侧面板高度，压缩审计诊断与运行日志空间。
- **解决方案**：
  - 将路线显隐区域改为横向排列，并放入矮的横向滚动容器。
  - 路线较少时直接横向铺开，路线较多时横向滚动，不再随路线数量向下扩张。
  - 补充 GUI smoke 测试，确认路线显隐布局保持横向。
